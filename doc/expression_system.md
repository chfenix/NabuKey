...
1. **待机 (Idle)**
   - 随机眨眼
   - 四处看(左右随机)
   - **新增**: 眨单眼(Wink)、眯眼(Squint)、瞪大眼(Wide Eyes)、左右扫视(Shifty Eyes)
   - 表情循环(可选,默认开启)
   - 循环周期:30秒自动切换
...
待机状态下，系统会随机触发不同的表情动作：

1. **正常 (NORMAL)** - 快速眨眼
2. **微笑 (SLIGHT_SMILE)** - 眼睛稍微缩小，保持1.5秒
3. **好奇 (CURIOUS)** - 随机向左或右看，保持1.5秒
4. **眨单眼 (WINK)** - 左眼或右眼单独眨动
5. **眯眼 (SQUINT)** - 双眼微眯，仿佛在思考或仔细看
6. **瞪大眼 (WIDE_EYES)** - 双眼瞬间放大，表现出惊讶或警觉
7. **左右扫视 (SHIFTY_EYES)** - 快速左右移动视线，表现出机灵感

### 触发机制

- **基础动作**: 每2-6秒随机执行一次动作
- **表情概率**: 
  - **30%** 概率触发随机表情（上述7种表情之一）
  - 70% 概率执行基础动作（眨眼或四处看）
- **随机性**: 表情类型和方向都是随机的，更加生动自然
...
- ✅ 新增待机表情: Wink, Squint, Wide Eyes, Shifty Eyes
- ✅ 提高待机表情触发概率至 30%
